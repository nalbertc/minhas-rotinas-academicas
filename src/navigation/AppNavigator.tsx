          ...
          {state.routes.map((route, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={onPress}
                activeOpacity={0.9}
                className={`flex-1 items-center justify-center ${!isFocused && 'scale-95'} ${isFocused && 'scale-[1.01]'} `}>
                {tabBarIcon &&
                  tabBarIcon({
                    focused: isFocused,
                    size: 22,
                ...
                <Text className={clsx(isFocused ? 'text-primary font-semibold' : 'text-gray-500 font-regular')}>
                  {label}
                </Text>
              </TouchableOpacity>
            );
